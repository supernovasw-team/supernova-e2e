/**
 * Backoffice CRUD 11 — Therapy (Terapias Guiadas): create, verify list, verify DB.
 *
 * Route: /categorias/terapias
 * DB table: terapias (@@map in Prisma schema)
 */
...
const UNIQUE = `E2E CRUD — Terapia ${Date.now()}`

test.describe.configure({ mode: 'serial' })

test.describe('/categorias/terapias CRUD — create, verify list, verify DB', () => {
  ...
  test('step 1: navigate + open create modal', async ({ page }) => {
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/11-01-terapia-list.png`, fullPage: true })
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/11-02-terapia-modal-open.png`, fullPage: true })
  ...
  test('step 2: fill all required fields', async ({ page }) => {
    ...
    await page.keyboard.type('Terapia criada automaticamente pelo E2E CRUD.')
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/11-03-terapia-form-filled.png`, fullPage: true })
  })
  ...
  test('step 3: submit and wait for list to show new item', async ({ page }) => {
    ...
    await page.screenshot({ path: `${SCREENSHOT_DIR}/11-04-terapia-created.png`, fullPage: true })
  })

  test('step 4: DB row exists in terapias', async () => {
    const rows = await dbAssert<{ id: string; name: string }>(
      config.db.url,
      `SELECT id, name FROM terapias WHERE name LIKE 'E2E CRUD%' ORDER BY id DESC LIMIT 1`,
      ['id', 'name'],
    )
    console.log('[db-assert] terapias row:', rows[0])
  })
})

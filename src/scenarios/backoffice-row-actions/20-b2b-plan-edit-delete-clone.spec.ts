/**
 * Row-action specs — B2B Plan Manager  (/b2b-plan-manager)
 * DB table: b2b_plans (Prisma @@map may vary — adjust if needed)
...
const SEED_NAME = 'E2E Seed — B2B Plan'
const EDIT_SUFFIX = ' EDITED'
const THROWAWAY = `E2E DELETE — B2B Plan ${Date.now()}`

test.describe.configure({ mode: 'serial' })

test.describe('/b2b-plan-manager row-actions', () => {
  ...
  test('step 1: edit seeded plan and DB-assert name updated', async ({ page }) => {
    ...
    await dbAssert(config.db.url,
      `SELECT name FROM b2b_plans WHERE name = '${SEED_NAME}${EDIT_SUFFIX}' LIMIT 1`,
      ['name'], (r) => r.name === SEED_NAME + EDIT_SUFFIX)
    await dbQuery(config.db.url,
      `UPDATE b2b_plans SET name = '${SEED_NAME}' WHERE name = '${SEED_NAME}${EDIT_SUFFIX}'`)
  })
  ...
  test('step 2: create throwaway plan + delete + DB-assert gone', async ({ page }) => {
    ...
    const rows = await dbQuery(config.db.url,
      `SELECT id FROM b2b_plans WHERE name = '${THROWAWAY}' LIMIT 1`)
  ...
  test('step 3: clone seeded plan and DB-assert new row exists', async ({ page }) => {
    ...
    // cleanup clone
    await dbQuery(config.db.url,
      `DELETE FROM b2b_plans WHERE name ILIKE '%${SEED_NAME}%' AND name != '${SEED_NAME}' AND created_at > NOW() - INTERVAL '5 minutes'`)
  })
})

/**
 * Row-action specs — Programas  (/categorias/programas)
 * DB table: programs  (reorder API: POST /api/reorderProgramas)
 */
...
const SEED_NAME = 'E2E Seed — Programa'
const EDIT_SUFFIX = ' EDITED'
const THROWAWAY = `E2E DELETE — Programa ${Date.now()}`

test.describe.configure({ mode: 'serial' })

test.describe('/categorias/programas row-actions', () => {
  ...
  test('step 2: edit seeded row and DB-assert', async ({ page }) => {
    ...
    await dbQuery(config.db.url,
      `UPDATE programs SET name = '${SEED_NAME}' WHERE name = '${SEED_NAME}${EDIT_SUFFIX}'`)
  })
  ...
  test('step 3: create throwaway + delete + DB-assert gone', async ({ page }) => {
    ...
    const rows = await dbQuery(config.db.url,
      `SELECT id FROM programs WHERE name = '${THROWAWAY}' LIMIT 1`)
  ...
  test('step 4: reorder top two rows and DB-assert order changed', async ({ page }) => {
    ...
    const before = await dbQuery(config.db.url,
      `SELECT id, "order" FROM programs ORDER BY "order" ASC NULLS LAST LIMIT 2`)

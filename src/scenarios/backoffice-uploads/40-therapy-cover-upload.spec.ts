/**
 * Spec 40 — Therapy edit page: Cover image upload via ContentHeader.
 *
 * Strategy:
 *  1. Create therapy, navigate to /categorias/terapia/edit/:id
 *  2. Upload test-image.png to <input#cover>
 *  3. Save, assert success toast
 *  4. DB assert: terapias.cover starts with 'data:image/'
 */
...
const UNIQUE_TITLE = `E2E Therapy Cover ${Date.now()}`

test.describe.configure({ mode: 'serial' })

test.describe('40 — therapy cover upload', () => {
  test('step 1: create therapy + navigate to edit', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/terapia\/edit\/\d+/, { timeout: 15_000 })
    await page.screenshot({ path: `${SCREENSHOT_BASE}/40-02-edit-page.png`, fullPage: true })
  })
  ...
  test('step 2: upload cover + save + assert DB', async ({ page }) => {
    ...
    if (rows.length === 0) {
      test.skip(true, 'Therapy not found in DB')
      return
    }
    const therapyId = rows[0][0]
    ...
    await dbAssert<{ cover: string }>(
      config.db.url,
      `SELECT cover FROM terapia WHERE id = ${therapyId};`,
      ['cover'],

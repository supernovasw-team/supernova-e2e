/**
 * Spec 43 — Article edit page: Thumbnail image upload via ContentHeader.
 *
 * Reuses article from spec 42 or creates new. Uploads to <input#thumbnail>.
 * DB assertion: artigos.thumbnail starts with 'data:image/'
 */
...
const UNIQUE_TITLE = `E2E Article Thumb ${Date.now()}`

test.describe.configure({ mode: 'serial' })

test.describe('43 — article thumbnail upload', () => {
  let articleId: string

  test('step 1: ensure article exists, navigate to edit', async ({ page }) => {
    await loginAsAdmin(page)

    const existing = await dbQuery<Record<string, string>>(
      config.db.url,
      `SELECT id FROM artigo WHERE name LIKE 'E2E%Article%' ORDER BY id DESC LIMIT 1;`,
    )

    if (existing.length > 0) {
      articleId = existing[0][0]
    } else {
    ...
    await page.screenshot({ path: `${SCREENSHOT_BASE}/43-01-edit-loaded.png`, fullPage: true })
  })
  ...
  test('step 2: upload thumbnail + save + assert DB', async ({ page }) => {
    ...
    if (rows.length === 0) {
      test.skip(true, 'No E2E article found')
      return
    }
    const id = rows[0][0]

    await page.goto(`/categorias/artigos/edit/${id}`)
    await expect(page.locator('body')).toBeVisible({ timeout: 20_000 })
    await page.waitForTimeout(2_000)
    ...
    await dbAssert<{ thumbnail: string }>(
      config.db.url,
      `SELECT thumbnail FROM artigo WHERE id = ${id};`,
      ['thumbnail'],

/**
 * Spec 47 — Programa edit page: Thumbnail image upload via ContentHeader.
 *
 * Reuses programa from spec 46 or creates new. Uploads to <input#thumbnail>.
 * DB assertion: programas.thumbnail starts with 'data:image/'
 */
...
const UNIQUE_TITLE = `E2E Programa Thumb ${Date.now()}`

test.describe.configure({ mode: 'serial' })

test.describe('47 — programa thumbnail upload', () => {
  let programaId: string

  test('step 1: ensure programa exists, navigate to edit', async ({ page }) => {
    await loginAsAdmin(page)

    const existing = await dbQuery<Record<string, string>>(
      config.db.url,
      `SELECT id FROM "Programa" WHERE name LIKE 'E2E%Programa%' ORDER BY id DESC LIMIT 1;`,
    )

    if (existing.length > 0) {
      programaId = existing[0][0]
    } else {
    ...
    await page.screenshot({ path: `${SCREENSHOT_BASE}/47-01-edit-loaded.png`, fullPage: true })
  })
  ...
  test('step 2: upload thumbnail + save + assert DB', async ({ page }) => {
    ...
    if (rows.length === 0) {
      test.skip(true, 'No E2E programa found')
      return
    }
    const id = rows[0][0]

    await page.goto(`/categorias/programa/edit/${id}`)
    await expect(page.locator('body')).toBeVisible({ timeout: 20_000 })
    await page.waitForTimeout(2_000)
    ...
    await dbAssert<{ thumbnail: string }>(
      config.db.url,
      `SELECT thumbnail FROM "Programa" WHERE id = ${id};`,
      ['thumbnail'],

...
export async function startBackend(opts: {
  ...
  if (existing) throw new Error(`Port ${opts.port} already in use by PID ${existing}. Kill it before starting backend.`)

  if (opts.logFile) mkdirSync(dirname(opts.logFile), { recursive: true })

  const child = spawn('npm', ['run', 'dev'], {
    cwd: opts.backendPath,
    detached: true,
    env: {
      ...process.env,
